...
import { getSavedWords, saveWords } from "./storage.js";
...
const WORD_ACTIONS = ["drag", "check", "delete"];
function isWordAction(value) {
    return WORD_ACTIONS.includes(value);
}
const syncControls = () => {
...
wordCloud.addEventListener("word-change", (event) => {
    console.log(`[word-cloud] renamed word: "${event.oldValue}" -> "${event.value}"`);
});
...
wordCloud.addEventListener("word-action-change", (event) => {
    console.log(`[word-cloud] word action: ${event.oldWordAction} -> ${event.wordAction}`);
    syncControls();
});
wordCloud.addEventListener("word-input-toggle", (event) => {
    console.log(`[word-cloud] word-input: ${event.oldWordInput} -> ${event.wordInput}`);
    syncControls();
});
wordCloud.addEventListener("physics-pause", (event) => {
    console.log(`[word-cloud] physics-paused: ${event.oldPhysicsPaused} -> ${event.physicsPaused}`);
});
